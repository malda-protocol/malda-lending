...
contract Operator is OperatorStorage, ImTokenOperationTypes, OwnableUpgradeable, HypernativeFirewallProtected {
     ...
     */
    function setMarketSupplyCaps(address[] calldata mTokens, uint256[] calldata newSupplyCaps) external onlyFirewallApprovedAllowEOA {
        require(
     ...
     */
    function setPaused(address mToken, ImTokenOperationTypes.OperationType _type, bool state) external onlyFirewallApprovedAllowEOA {
        if (state) {
            require(
     ...
     * @inheritdoc IOperatorDefender
     */
    function beforeMTokenMint(address mToken, address minter, address receiver) external view override onlyAllowedUser(minter) ifNotBlacklisted(minter) ifNotBlacklisted(receiver) onlyAllowedUser(minter) {
    ...
    function beforeMTokenRedeem(address mToken, address redeemer, uint256 redeemTokens) external view override onlyAllowedUser(redeemer) ifNotBlacklisted(redeemer) {
