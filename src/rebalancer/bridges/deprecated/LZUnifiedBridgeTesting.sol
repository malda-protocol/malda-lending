...
contract LZUnifiedBridgeTesting is BaseBridge, IBridge {
    ...
    function setBridgeContract(address underlying, address bridgeContract) external  {
        if (msg.sender != admin) revert LZBridge_BadFrom();
        bridgeContracts[underlying] = bridgeContract;
        emit BridgeContractSet(underlying, bridgeContract);
    }

    function setOftExecutorContract(address underlying, address bridgeContract) external {
        if (msg.sender != admin) revert LZBridge_BadFrom();
