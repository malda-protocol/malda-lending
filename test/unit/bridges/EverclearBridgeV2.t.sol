...
contract EverclearBridgeV2Test is Test {
    ...
    function test_SetEverclearFeeAdapter_RevertWhenCallerNotGuardian() public {
        vm.expectRevert(BaseBridge.BaseBridge_NotAuthorized.selector);
        bridge.setEverclearFeeAdapter(address(0xBEEF));
    }

    function test_SetEverclearFeeAdapter_RevertWhenZeroAddress() public {
        vm.prank(guardian);
        vm.expectRevert(EverclearBridgeV2.Everclear_AddressNotValid.selector);
        bridge.setEverclearFeeAdapter(address(0));
    }

    function test_SetEverclearFeeAdapter_Updates() public {
    ...
    function test_SendMsg_RevertWhenMaxSlippageExceeded(uint96 amountRaw, uint256 amountOutMinRaw) public {
        IntentInput memory input = _defaultInput();
