...
/// @title DeployGasHelper
/// @author Merge Layers Inc.
/// @notice Single deployment script that deploys DefaultGasHelper
contract DeployGasHelper is DeploymentScriptBase {
    ////////////////////////////////////////////////////////////
    //                        Structs                         //
    ////////////////////////////////////////////////////////////
    ...
    error InvalidSalt();
    /// @notice Error thrown when gas helper address is invalid
    error InvalidGasHelperAddress();
    ...
    function _deployAndAssertResult(bytes memory deployConfig) internal override returns (address gasHelperAddress) {
        ...
        gasHelperAddress = deployer.precompute(salt);
        if (gasHelperAddress.code.length == 0) {
            bytes memory callData = abi.encodeWithSelector(
                deployer.create.selector,
                salt,
                abi.encodePacked(type(DefaultGasHelper).creationCode, abi.encode(cfg.owner))
            );
            ...
            gasHelperAddress = abi.decode(returnData, (address));
        }

        // Requirements: gasHelperAddress should not be the zero address; gasHelperAddress code length should be greater than
        // zero; owner should equal cfg.owner.
        require(gasHelperAddress != address(0), InvalidGasHelperAddress());
        require(gasHelperAddress.code.length > 0, InvalidGasHelperAddress());
        require(DefaultGasHelper(gasHelperAddress).owner() == cfg.owner, InvalidOwner());
    }
    ...
    function _defaultConfigPath() internal pure override returns (string memory path) {
        path = "config/deploy/single/DeployGasHelper.config.json";
    }

    /// @inheritdoc ScriptBase
    function _defaultOutputPath() internal pure override returns (string memory path) {
        path = "config/deploy/single/DeployGasHelper.output.json";
    }
}
