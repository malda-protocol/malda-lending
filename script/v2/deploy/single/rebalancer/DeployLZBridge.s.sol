...
/// @title DeployLZBridge
/// @author Merge Layers Inc.
/// @notice Single deployment script that deploys LZUnifiedBridge
contract DeployLZBridge is DeploymentScriptBase {
    ////////////////////////////////////////////////////////////
    //                        Structs                         //
    ////////////////////////////////////////////////////////////
    ...
    struct DeployConfig {
        ...
        address roles;
        /// @notice Config value for endpoint
        address endpoint;
    ...
    error InvalidRoles();
    /// @notice Error thrown when endpoint is invalid
    ...
    function _deployAndAssertResult(bytes memory deployConfig) internal override returns (address bridgeAddress) {
        ...
        if (bridgeAddress.code.length == 0) {
            bytes memory callData = abi.encodeWithSelector(
                ...
                salt,
                abi.encodePacked(type(LZUnifiedBridge).creationCode, abi.encode(cfg.roles, cfg.endpoint))
            );
        ...
        LZUnifiedBridge bridge = LZUnifiedBridge(bridgeAddress);
        // Requirements: roles should equal cfg.roles; endpoint should equal cfg.endpoint.
        require(address(bridge.roles()) == cfg.roles, InvalidRoles());
        require(bridge.ENDPOINT() == cfg.endpoint, InvalidEndpoint());
    }
        ...
        returns (string memory serialized)
    {
        ...
        vm.serializeAddress(json, "endpoint", cfg.endpoint);
    ...
    function _loadAndValidateConfig() internal view override returns (bytes memory deployConfig) {
        ...
        cfg.endpoint = _readAndLogAddress(json, "endpoint");
        ...
        require(cfg.endpoint != address(0), InvalidEndpoint());
    ...
    function _defaultConfigPath() internal pure override returns (string memory path) {
        path = "config/deploy/single/DeployLZBridge.config.json";
    }

    /// @inheritdoc ScriptBase
    function _defaultOutputPath() internal pure override returns (string memory path) {
        path = "config/deploy/single/DeployLZBridge.output.json";
    }
}
