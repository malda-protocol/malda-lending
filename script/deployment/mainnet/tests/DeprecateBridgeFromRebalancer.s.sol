...
contract DeprecateBridgeFromRebalancer is DeployBaseRelease {
    ...
    function run() public {
        ...
        for (uint256 i = 0; i < networks.length; i++) {
            ...
            Rebalancer(rebalancerContract).setWhitelistedBridgeStatus(address(bridgeContract), false);
            vm.stopBroadcast();

            console.log("Setting allowed tokens per bridge");
            address[] memory tknsArr = tokens[crtChainId];
            vm.startBroadcast(key);
            Rebalancer(rebalancerContract).setAllowedTokens(address(bridgeContract), tknsArr, false);
            vm.stopBroadcast();
