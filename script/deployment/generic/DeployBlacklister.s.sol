...
contract DeployBlacklister is Script {
    function run(Deployer deployer, address roles, address owner) public returns (address) {
        ...
        bytes32 salt = getSalt("BlacklisterV1.0.0");
        ...
        if (created.code.length == 0) {
            ...
            created = deployer.create(
                salt, abi.encodePacked(type(Blacklister).creationCode, abi.encode(owner, roles))
            );
            vm.stopBroadcast();
            console.log("Blacklister deployed at:", created);
        } else {
            console.log("Using existing Blacklister at: %s", created);
        }
