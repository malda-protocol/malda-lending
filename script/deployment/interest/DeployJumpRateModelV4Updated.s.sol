 ...
 *     --sig "run((uint256,string,uint256,uint256,uint256,uint256))" \
 *     "(750000000000000000,'ExampleName',2102400,20000000000000000,100000000000000000,500000000000000000)" \
 *     --broadcast
 */
contract DeployJumpRateModelV4Updated is Script {
    ...
    function run() public view {
        ...
        bytes32 salt =
            getSalt(string.concat(data.name, "JumpRateModelV1.0.5-0xa31963C753f277f7d82d98F56b2C374256925eB7"));
        ...
        bytes memory creationCode = abi.encodePacked(type(JumpRateModelV4).creationCode, constructorArgs);
        bytes memory callData = abi.encodeWithSelector(deployer.create.selector, salt, creationCode);
